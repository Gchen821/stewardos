"""Unified LLM client with streaming, tool-calling, and async support."""
...
from app.core.llm.response import LLMResponse, StreamStats, LLMToolResponse
from app.core.llm.adapters import create_adapter, BaseLLMAdapter
...
class HelloAgentsLLM:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.fallback_model = os.getenv("LLM_FALLBACK_MODEL_ID", "").strip() or None
    ...
    def _create_adapter_for_model(self, model: str) -> BaseLLMAdapter:
    ...
    def _invoke_with_retry_and_fallback(
        self,
        ...
        for model_name in models_to_try:
            adapter = self._adapter if model_name == self.model else self._create_adapter_for_model(model_name)
            for attempt in range(max(0, self.retry_max) + 1):
                try:
                    response = adapter.invoke(messages, **call_kwargs)
                    response.model = model_name
                    return response
                except Exception as exc:
                    last_error = exc
                    retryable = self._is_retryable_rate_limit_error(exc)
                    has_retry = retryable and attempt < max(0, self.retry_max)
                    if has_retry:
                        self._sleep_backoff(attempt)
                        continue
                    if retryable and model_name != models_to_try[-1]:
                        break
                    raise
        if last_error is not None:
            raise last_error
        raise HelloAgentsException("LLM 调用失败：未知错误")
    ...
    def invoke(self, messages: List[Dict[str, str]], **kwargs) -> LLMResponse:
        ...
        return self._invoke_with_retry_and_fallback(messages, call_kwargs)
    ...
    def stream_invoke(self, messages: List[Dict[str, str]], **kwargs) -> Iterator[str]:
        ...
        for model_name in models_to_try:
            ...
            for attempt in range(max(0, self.retry_max) + 1):
                try:
                    for chunk in adapter.stream_invoke(messages, temperature=temperature, **call_kwargs):
                        yield chunk
                    if hasattr(adapter, "last_stats"):
                        self.last_call_stats = adapter.last_stats
                    if self.last_call_stats is not None:
                        self.last_call_stats.model = model_name
                    return
                except Exception as exc:
                    last_error = exc
                    retryable = self._is_retryable_rate_limit_error(exc)
        ...
        if last_error is not None:
            raise last_error
        raise HelloAgentsException("LLM 流式调用失败：未知错误")
